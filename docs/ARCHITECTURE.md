...
- **运行取消（cooperative cancellation，2026-06-02 `88afa84`）**：删除一个**正在跑**的 campaign 要真停掉它。observer 的 run 是 fire-and-forget 后台线程 + `claude -p` 子进程；**Python daemon 线程不能被外部强杀**（只能 cooperative 退出）。机制：删除删 `campaigns` 行（`delete_campaign`）→ **Runner 每个 stage 边界查 `index.campaign_exists`**，没了就停 + 清掉自己写的残留 + return `cancelled`。关键：**Runner 从不写 `campaigns` 表**（只写 `stage_state`），删除信号才稳定不被重建；run 开头用 `cancellable` 快照区分"从未注册（cli/测试）"vs"注册后被删（取消）"，避免误停。**职责划分**：backend 提供这个 cooperative 钩子（让删除能停 run、不再写回"复活"）；**立即**中断当前 stage 的 `claude -p`（不等它跑完）+ 前端"停止"按钮/列表刷新 = observer/HAPI（api 持线程·进程句柄做 group-kill）。验证：real run 跑到 disease-overview 时删除 → 下个 stage 边界停、`stage_state` 清空不再增长、claude 子进程归 0。**运维教训**：重启 uvicorn 必须 **group-kill**（`kill -- -<pgid>`）——普通 kill 主进程会留下 `claude -p` 子进程孤儿 spin（coder-loop 的 "process-GROUP kill (must)" 教训，我自己踩过一次）。
