...
1. **疾病 input gate**（`intake.py`，`intake_fn` 注入进 `Runner.run`，`--real` only）：用户输入疾病名先过守门——脚本 gate（空/超长/纯符号）→ claude -p（翻译成英文 + 调 OT `search_disease` 查 EFO，禁 CC 内置工具）→ typed `DiseaseIntake`（accepted/normalized_en/efo_id/reason）。**只放真实疾病进流程**，归一化的英文名喂下游；拒绝时 stage-0 标 exhausted+reason（web 可见）。实测：`老年黄斑变性`→`age-related macular degeneration`(EFO_0001365)、`type 2 diabetes`→`…mellitus`(MONDO_0005148) 放行；`帮我写首诗`、`'; DROP TABLE --` 拒（后者识别为注入串）。**收口位置教训（`ab8ef28`）**：初版只加在 `cli.main`，但 web UI 触发走 `api._run_pipeline`（另建一个 Runner）绕过了它——`帮我写首诗` 从 web 直接进了 stage-0。守门必须下沉到**两入口（cli + api）唯一共同收口 `Runner.run`**（注入 `intake_fn`，像 `judge_fn`），并补上 api 漏传的 `planner_fn`。
