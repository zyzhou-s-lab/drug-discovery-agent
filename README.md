...
- [x] **持久化/恢复策略**：CC 只给 session 级恢复（resume 会重复 tool call），durable 必须自建 → ARCHITECTURE §3.8。**(c)→(a) 渐进**：快节点先跑通(c)，接 GPU/slurm 长算上(a) 手写轻量（idempotency key + jobid 重连）；(b) Temporal 留待生产化
